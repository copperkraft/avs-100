app.factory("levelSRV", function () { //TODO переписать на получение JSON или генерацию в
    ...
    return {
        "1": {
            ...
            description: "Приветствуем вас в серии обучающих заданий. В этих заданиях вам будет дан набор чисел, которые должны быть поданы на вывод программы в процессе ее выполнения. Первое задание предельно простое. Достаточно написать и запустить (нажатием кнопки run) программу, которая выведет три числа. Для этого вам понадобится команда вывода OUT. Для того, чтобы вывести число 12 достаточно написать 'OUT 12'. Удачи.",
        ...
        "2": {
            ...
            description: "Одной из особенностей этого симулятора является то, что когда выполнение команд доходит до последней строки, выполнение программы не останавливается, а продолжается с первой строки. Поэтому для выполения этого задания подойдет программа, написанная для прохождения предыдущего задания.",
        ...
        "13": {
            ...
            expectations: (function (input) {
                var output = [];
                for (var i = 0; i < 5; i++) {
                    output.push(Math.abs(input[i*2] - input[i*2+1]));
                }
                return output;
            }(inputArray.map(function (inp) {
                return Math.floor(inp/2);
        ...
        "14": {
            ...
            expectations: (function (input) {
                var output = [];
                for (var i = 0; i < 5; i++) {
                    output.push(input[input[i+5]%5]);
                }
                return output;
            }(inputArray)),
            description: "Для работы с большим количеством данных необходимо использовать массивы. В симуляторе массивов в привычном понимании нет. Но вместо этого есть два способа для работы с большим объемом данных. Первый из них - обращение к регистру \"по ссылке\". Для этого существует команды MRD и MWR. Работа этих команды в целом аналогична работе команды MOV, за исключением того, что в команде MWR первый аргумент воспринимаются не как значение, а как номер регистра, а в MRD так воспринимается второй. Например 'MWR 7 1' запишет значение 1 в R7. А 'MRD R0 7' запишет значение из R7 в R0. Для того, чтобы использовать это как массив можно последовательно изменять регистр \"указателя\" на еденицу и записывать или считывать используя этот регистр как первый аргумент MWR или второй аргумент MRD соответственно ",
            specification: "На вход поданы 5 чисел. Затем 5 номеров. Необходимо вывести соответствующие индексам числа. Индексация начинается с 0"
        }
    };
});